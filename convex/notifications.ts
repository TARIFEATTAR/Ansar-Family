 ...
 * All logic is inlined to avoid cross-reference type issues.
 */
export const sendWelcomeContact = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.phone) {
      ...
      if (!accountSid || !authToken || !fromNumber) {
        console.error("❌ Missing Twilio env vars for contact SMS");
        await ctx.runMutation(internal.messages.logMessage, {
          type: "sms",
          recipientId,
        ...
        try {
          ...
          const message = getWelcomeContactSMS(firstName, args.role);
          ...
          const errorMessage = error instanceof Error ? error.message : "Unknown error";
          console.error(`❌ Contact SMS failed:`, errorMessage);
          await ctx.runMutation(internal.messages.logMessage, {
            type: "sms",
            recipientId,
            recipientPhone: args.phone,
    ...
    if (args.email) {
      ...
      if (!apiKey) {
        console.error("❌ Missing Resend API key for contact email");
        await ctx.runMutation(internal.messages.logMessage, {
          type: "email",
          recipientId,
        ...
        try {
          const emailContent = getWelcomeContactEmail(firstName, args.role, args.organizationName);
          ...
          console.log(`✅ Contact email sent to ${args.email}, ID: ${data.id}`);
        } catch (error) {
          const errorMessage = error instanceof Error ? error.message : "Unknown error";
          console.error(`❌ Contact email failed:`, errorMessage);
          await ctx.runMutation(internal.messages.logMessage, {
            type: "email",
            recipientId,
            recipientEmail: args.email,
